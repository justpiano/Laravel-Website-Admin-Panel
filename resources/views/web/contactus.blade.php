    ...
    <div class="contact-us">
        <div class="container">
            <div class="contact-content">
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-auto left-side">
                        <div class="row mb-4">
                            <div class="col-xl-3 col-lg-6 col-sm-6 my-1">
                                <div class="card border-0 shadow rounded-3 p-3 h-100">
                                    <h3> <i class="fa-solid fa-envelope"></i> {{ trans('labels.email') }}</h3>
                            ...
                            <div class="col-xl-3 col-lg-6 col-sm-6 my-1">
                                <div class="card border-0 shadow rounded-3 p-3 h-100 ">
                                    <h3> <i class="fa-solid fa-phone"></i> {{ trans('labels.mobile') }}</h3>
                            ...
                            <div class="col-xl-3 col-lg-6 col-sm-6 my-1">
                                <div class="card border-0 shadow rounded-3 p-3 h-100">
                                    <h3> <i class="fa-solid fa-location-dot"></i> {{ trans('labels.address') }}</h3>
                                    <a href="javascript:void(0)">{{@Helper::appdata()->address }}</a>
                                </div>
                            </div>
                            <div class="col-xl-3 col-lg-6 col-sm-6 my-1">
                                <div class="card border-0 shadow rounded-3 p-3 h-100">
                                    <h3> <i class="fa-solid fa-clock"></i> {{ trans('labels.working_hours') }}  </h3>
                                    <h5 class="text-muted">{{ ucfirst($timedata->day) }} <span
                                    ...
                                    <p>{{ $timedata->open_time }} <b>{{ trans('labels.to') }}</b> {{ $timedata->break_start
                                        }}</p>
                                    <p>{{ $timedata->break_end }} <b>{{ trans('labels.to') }}</b> {{ $timedata->close_time
                                        }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-12 col-sm-12 col-auto right-side">
                        <form method="POST" action="{{ route('createcontact') }}">
                            @csrf
                            <div class="row">
                                <p class="text-center">{{ trans('labels.contactus_heading') }}</p>
                                <span class="text-muted text-center">{{ trans('labels.contactus_description') }}</span>
                            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-danger" data-bs-dismiss="modal">{{ trans('labels.close')
                    }}</button>
            </div>
        </div>
    </div>
