        ...
        <nav class="navbar navbar-expand-lg sticky-top p-0">
            <div class="container navbar-container">
                ...
                <div class="buttons d-flex align-items-center">
                    <button class="btn hamburger-lines px-1" type="button" data-bs-toggle="offcanvas"
                    ...
                    <li class="nav-item dropdown list-unstyled">
                        ...
                        <ul class="dropdown-menu dropdown-menu-custom" aria-labelledby="menudropdown" id="style-3">
                            @foreach (Helper::get_categories() as $category)
                                <li><a class="dropdown-item text-dark @if (isset($_GET['category']) && $_GET['category'] == $category->slug) active @endif"
                                        href="{{ URL::to('/menu?category=' . $category->slug) }}">{{ $category->category_name }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </li>
                            ...
                            <ul class="navbar-nav my-3 px-3">
                                <li class="nav-item dropdown">
                                    <div class="dropup">
                                        ...
                                        <ul class="dropdown-menu {{session()->get('direction') == 'rtl' ? 'text-end' : 'text-start'}}" aria-labelledby="profiledropdown">
                                            <li><a class="dropdown-item text-dark"
                                                    href="{{ route('user-profile') }}"><i
                                                        class="mx-2 fa-regular fa-user"></i>{{ trans('labels.my_profile') }}</a>
                                            </li>
                                            <li><a class="dropdown-item text-dark"
                                                    href="{{ route('order-history') }}"><i
                                                        class="mx-2 fa fa-list-check"></i>{{ trans('labels.my_orders') }}</a>
                                            </li>
                                            <li><a class="dropdown-item text-dark"
                                                    href="{{ route('user-favouritelist') }}"><i
                                                        class="mx-2 fa fa-heart-circle-check"></i>{{ trans('labels.favourite_list') }}</a>
                                            </li>
                                            <li><a class="dropdown-item text-dark"
                                                    href="{{ route('user-changepassword') }}"><i
                                                        class="mx-2 fa fa-key"></i>{{ trans('labels.change_password') }}</a>
                                            </li>
                                            <li><a class="dropdown-item text-dark" href="{{ route('address') }}"><i
                                                        class="mx-2 fa-solid fa-location-crosshairs"></i>{{ trans('labels.my_addresses') }}</a>
                                            </li>
                                            <li><a class="dropdown-item text-dark" href="{{ route('user-wallet') }}"><i
                                                        class="mx-2 fa fa-wallet"></i>{{ trans('labels.my_wallet') }}</a>
                                            </li>
                                            ...
                                            <li><a class="dropdown-item text-dark" href="javascript:void(0)"
                                                    onclick="logout('{{ route('logout') }}','{{ trans('messages.are_you_sure_logout') }}','{{ trans('labels.logout') }}')"><i
                                                        class="mx-2 fa fa-arrow-right-from-bracket"></i>{{ trans('labels.logout') }}</a>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
            ...
            <div class="navbar-collapse collapse">
                ...
                <div class="d-flex align-items-center nav-sidebar-d-none">
                    ...
                    <div class="cart-area mx-2 d-block">
                        <a href="{{ route('cart') }}"href="{{ route('login') }}"
                    ...
                    <div class="mx-3">
                        ...
                        @else
                            <div class="mx-4">
                                <a href="{{ route('login') }}"
                                    class="btn btn-sm btn-primary log-btn">{{ trans('labels.signin') }}</a>
                            </div>
                        @endif
                    </div>
                    @if (session()->get('direction') == '')
